...
template class PipetteNeedleReagentBin<0>;
template class PipetteNeedleReagentBin<1>;
template class PipetteNeedleReagentBin<2>;
template class PipetteNeedleReagentBin<3>;
template class PipetteNeedleReagentBin<4>;
template class PipetteNeedleReagentBin<5>;
template class PipetteNeedleReagentBin<6>;
...
template <size_t BinIdx>
bool PipetteNeedleReagentBin<BinIdx>::Save_fixtureParams() {
    /// 保存移液针
    {
        auto&& needle                               = Device::instance();
        needle.fix.ReagentBinList[BinIdx].yPosition = m_params.yPosition;
        needle.fix.ReagentBinList[BinIdx].zPosition = m_params.zPosition;
...
template <size_t BinIdx>
bool PipetteNeedleReagentBin<BinIdx>::Save_fixtureParams_Without_Z() {
    /// 保存移液针
    {
        auto&& needle                               = Device::instance();
...
template <size_t BinIdx>
void PipetteNeedleReagentBin<BinIdx>::op_clockwise() {
...
template <size_t BinIdx>
void PipetteNeedleReagentBin<BinIdx>::op_anticlockwise() {
    int num                  = get_inputNum();
    reagent_params.position += num;
    exec_fixtureMove_ReagentBin();
}
