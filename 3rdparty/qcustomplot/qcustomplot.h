...
enum Interaction {
    ...
    ,
    iMultiSelect =
        0x004  ///< <tt>0x004</tt> The user can select multiple objects by holding the modifier set
               ///< by \ref QCustomPlot::setMultiSelectModifier while clicking
    ,
    ...
    iSelectPlottablesBeyondAxisRect =
        0x100  ///< <tt>0x100</tt> When performing plottable selection/hit tests, this flag extends
               ///< the sensitive area beyond the axis rect
};
...
extern const QMetaObject
    staticMetaObject;  // in moc-run we create a static meta object for QCP "fake" object. This line
                       // is the link to it via QCP::staticMetaObject in normal operation as
                       // namespace
...
protected:
    ...
    QColor              mColor;
    int                 mPadding;
    double              mRotation;  // this is the rotation applied uniformly to all labels, not the
                                    // heterogeneous rotation in amCircularRotated mode
...
void QCPDataContainer<DataType>::add(const QCPDataContainer<DataType> &data) {
    ...
    } else  // don't need to prepend, so append and merge if necessary
    {
        ...
        if (oldSize > 0 && !qcpLessThanSortKey<DataType>(
                               *(constEnd() - n - 1),
...
PlottableType *QCustomPlot::plottableAt(const QPointF &pos, bool onlySelectable,
    ...
    foreach (QCPAbstractPlottable *plottable, mPlottables) {
        ...
        if (!currentPlottable ||
            (onlySelectable &&
             !currentPlottable
                  ->selectable()))  // we could have also passed onlySelectable to the selectTest
                                    // function, but checking here is faster, because we have access
                                    // to QCPAbstractPlottable::selectable
            continue;
...
ItemType *QCustomPlot::itemAt(const QPointF &pos, bool onlySelectable) const {
    ...
    foreach (QCPAbstractItem *item, mItems) {
        ...
        if (!currentItem ||
            (onlySelectable &&
             !currentItem->selectable()))  // we could have also passed onlySelectable to the
        ...
        if (!currentItem->clipToAxisRect() ||
            currentItem->clipRect().contains(
                pos.toPoint()))  // only consider clicks inside axis cliprect of the item if
                                 // actually clipped to it
        {
...
public:
    ...
    */
    enum NanHandling {
        nhNone  ///< NaN data points are not explicitly handled and shouldn't occur in the data
                ///< (this gives slight performance improvement)
        ,
...
class QCP_LIB_DECL QCPErrorBars
    : public QCPAbstractPlottable
...
public:
    ...
    */
    ...
    virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details = 0)
        const;  // actually introduced in QCPLayerable as non-pure, but we want to force
